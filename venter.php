<?php
    ...
    //Si l'utilisateur a déja créer ce produit, il va ajouter sur le quantité au lieu de créer un nouveau
    if($res){
        ...
        define('TARGET', '/volsme/user1x/users/na18a005/public_html/photos_produits/');    // Repertoire cible
        ...
        if(!is_dir(TARGET)) {
          ...
          $message = 'Directory doesnt exist';
        }
        if(!empty($_POST)){
          ...
          if(!empty($_FILES['photo']['name'])){
		    ...
		    $miniImage = resize_image(TARGET.$nomImage, 200, 200);
		    if(imagejpeg($miniImage, TARGET.$nomImage."_mini")){
		      $message = 'Upload + mini réussi';
		    } else {
